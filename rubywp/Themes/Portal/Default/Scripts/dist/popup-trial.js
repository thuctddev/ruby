...
function checkShowCaptcha(popupType, type, storeName, source, preferredService, phoneNumber) {
    $.ajax({
        ...
        success: function (t) {
            ...
            if (popupType == "fnb") {
                createModalFnB(phoneNumber, t.CountryCode);
            } else {
...
function createCaptcha($form) {
    ...
    ctx.fillText(captcha.join(""), 45, 30);
...
function createModal(type, storeName, source, preferredService, location) {
    var $modal = $('<div id="trialPopupModal" class="modal fade modal-register modal-register-default" role="dialog">'
        + '<button type="button" class="close" data-dismiss="modal" aria-label="Close"><svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 512.001 512.001"  xml:space="preserve"><g><path d="M284.286,256.002L506.143,34.144c7.811-7.811,7.811-20.475,0-28.285c-7.811-7.81-20.475-7.811-28.285,0L256,227.717 L34.143,5.859c-7.811-7.811-20.475-7.811-28.285,0c-7.81,7.811-7.811,20.475,0,28.285l221.857,221.857L5.858,477.859 c-7.811,7.811-7.811,20.475,0,28.285c3.905,3.905,9.024,5.857,14.143,5.857c5.119,0,10.237-1.952,14.143-5.857L256,284.287 l221.857,221.857c3.905,3.905,9.024,5.857,14.143,5.857s10.237-1.952,14.143-5.857c7.811-7.811,7.811-20.475,0-28.285 L284.286,256.002z"/></g></svg></button>'
        + '<div class="modal-dialog" role="document"><div class="modal-content"><div class="roof-store">'
        + '<img src="https://www.sapo.vn/Themes/Portal/Default/StylesV2/images/fnb/roof-store.png"></div><div class="modal-body"></div></div></div></div>'),
        $modalbody = $modal.find('.modal-body'),
        $form, $title;
    switch (type) {
        case 1:
            $title = "<h2>Dùng thử Sapo miễn phí 7 ngày</h2><p>+100,000 doanh nghiệp & chủ shop tin dùng</p>";
            break;
        case 2:
            $title = "<h2>Dùng thử website bán hàng Sapo Web</h2><p>Miễn phí 7 ngày với đầy đủ tính năng</p>";
            break;
        case 3:
            $title = "<h2>Dùng thử phần mềm quản lý fanpage Sapo Social</h2><p>Được tích hợp miễn phí trên Sapo</p>";
            break;
        case 4:
            $title = "<h2>Dùng thử phần mềm quản lý bán hàng Sapo POS</h2><p>Miễn phí 7 ngày với đầy đủ tính năng</p>";
            break;
        case 5:
            $title = "<h2>Dùng thử Sapo Omnichannel miễn phí 7 ngày</h2><p>+100,000 doanh nghiệp & chủ shop tin dùng</p>";
            break;
        case 6:
            $title = "<h2>Dùng thử Sapo GO miễn phí 7 ngày</h2><p>+100,000 doanh nghiệp & chủ shop tin dùng</p>";
            break;
        case 7:
            $title = "<h2>Dùng thử Sapo miễn phí 7 ngày</h2><p>+100,000 doanh nghiệp & chủ shop tin dùng</p>";
            break;
    ...
    var $phoneNumber = $("<input/>", { type: "tel", pattern: "^\\d{8,11}$", maxlength: "15", id: "PhoneNumber", class: "form-control form-input", name: "PhoneNumber", required: "" });
    var $email = $("<input/>", { type: "email", pattern: "^([a-zA-Z0-9_\\-\\.\\+]+)@((\\[[0-9]{1,3}\\.[0-9]{1,3}\\.[0-9]{1,3}\\.)|(([a-zA-Z0-9\\-]+\\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\\]?)$", maxlength: "50", id: "Email", class: "form-control form-input", name: "Email", required: "" });
    if (jQuery.ui) {
        var $password = $("<input/>", { type: "password", pattern: "^.{6,64}$", maxlength: "64", id: "Password", class: "form-control form-input", name: "Password", required: "" });
        $password.autocomplete({ source: "" });
    } else {
        var $password = $("<input/>", { type: "password", pattern: "^.{6,64}$", maxlength: "64", id: "Password", class: "form-control form-input", name: "Password", required: "", autocomplete: "store-password" });
    }
    //City & District
    ...
    $password = $formGroup.clone().append($password).append($label.clone().attr("for", "Password").text("Mật khẩu đăng nhập cửa hàng"));
    ...
    var $rowList = $row.clone().append($col6.clone().append($phoneNumber)).append($col6.clone().append($email));
    var $rowList2 = $row.clone().append($col6.clone().append($storeName)).append($col6.clone().append($password));
    var $rowList3;

    // Add input to form
    $form.append($fullName).append($rowList).append($rowList2);
    ...
    if (type != 2 && type != 3 && type != 4 && type != 5 && type != 6 && type != 7) {
        ...
        $rowList3 = $row.clone().append($col6.clone().append($selectCity)).append($col6.clone().append($select));
        $form.append($rowList3);
    } else if (type == 7) {
        ...
        $rowList3 = $row.clone().append($col6.clone().append($selectCity)).append($col6.clone().append($selectOmni));
        $form.append($rowList3);
    } else {
    ...
    $modal.find('input[type="text"], input[type="tel"], input[type="email"], input[type="password"]').on('change keyup paste input', function () {
        var input = $(this);
        if (input.val() == '') {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.result) {
                        ...
                        $form.find('.btn-registration').removeClass('btn-loading');
                        if (data.error && data.error.includes('capcha')) {
                            modalCaptchaRedirect(buildRedirectUrlCaptcha(type, $form));
                        } else {
                            if (data.error) {
                ...
                error: function (data) {
                    $form.find('.btn-registration').removeClass('btn-loading');
                    if (data.responseJSON && data.responseJSON.error.includes('capcha')) {
                        modalCaptchaRedirect(buildRedirectUrlCaptcha(type, $form));
                    } else {
...
function modalCaptchaRedirect(url) {
    var $modal = $('<div id="modalCaptchaRedirect" class="modal fade modal-register-success-default" role="dialog">'
        + '<div class="modal-dialog modal-captcha" role="document"><div class="modal-content"></div></div>'),
        $modalContent = $modal.find('.modal-content');

    // Add form vao modal
    var $htmlBody = '<h5>Đăng ký dùng thử</h5><p>Số lần đăng ký của bạn đã vượt quá giới hạn. Bạn hãy nhập mã an toàn để tiếp tục đăng ký dùng thử</p><div class="action"><a href="' + url + '" rel="nofollow">OK</a></div>';
    $modalContent.append($htmlBody);
...
function buildRedirectUrlCaptcha(type, form) {
    var url = redirectUri + "?Type=" + type;
    $(form).find('input').each(function () {
        var item = $(this);
        if (item.val() != "" && item.val() != null && item.attr('name') != "Type" && item.attr('name') != "Password" && item.attr('name') != "CaptchaInput") {
    ...
    if ($(form).find('select').length > 0) {
        $(form).find('select').each(function () {
            var item = $(this);
            if (item.attr('name') != "CityDistrict") {
                url += "&" + item.attr('name') + "=" + item.val();
            }
        });
    }
...
function showModalTrialFnB(e) {
    if (popupOpen == false) {
        ...
        checkShowCaptcha("fnb", "", "", "", "", phoneNumber);
    }
}

function createModalFnB(phoneNumber, location) {
    var $modal = $('<div id="trialPopupModalFnB" class="modal fade modal-register modal-register-default" role="dialog">'
        + '<button type="button" class="close" data-dismiss="modal" aria-label="Close"><svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 512.001 512.001"  xml:space="preserve"><g><path d="M284.286,256.002L506.143,34.144c7.811-7.811,7.811-20.475,0-28.285c-7.811-7.81-20.475-7.811-28.285,0L256,227.717 L34.143,5.859c-7.811-7.811-20.475-7.811-28.285,0c-7.81,7.811-7.811,20.475,0,28.285l221.857,221.857L5.858,477.859 c-7.811,7.811-7.811,20.475,0,28.285c3.905,3.905,9.024,5.857,14.143,5.857c5.119,0,10.237-1.952,14.143-5.857L256,284.287 l221.857,221.857c3.905,3.905,9.024,5.857,14.143,5.857s10.237-1.952,14.143-5.857c7.811-7.811,7.811-20.475,0-28.285 L284.286,256.002z"/></g></svg></button>'
        + '<div class="modal-dialog" role="document"><div class="modal-content"><div class="roof-store">'
        + '<img src="/Themes/Portal/Default/StylesV2/images/fnb/roof-store.png"></div><div class="modal-body"></div></div></div></div>'),
        $modalbody = $modal.find('.modal-body'),
        $form, $title = "<h2>Đăng ký dùng thử Sapo FnB</h2><p>Miễn phí 7 ngày với đầy đủ tính năng</p>";
    ...
    $modal.find('input[type="text"], input[type="tel"], input[type="email"]').on('change keyup paste input', function () {
        var input = $(this);
        if (input.val() == '') {
            input.removeClass('valid');
        } else {
...
function modalVerifiedOTP() {
    var $modal = $('<div id="modalVerifiedOTP" class="modal fade modal-login" role="dialog">'
        + '<div class="modal-dialog" role="document"><div class="modal-content"><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span class="ti-close"></span></button><div class="modal-body login-form"></div></div></div>'),
        $modalbody = $modal.find('.modal-body'),
        $modalTitle = '<h4 class="modal-title"><img src="/Themes/Portal/Default/Styles_New/Images/logo/Sapo-logo.svg?v=201807290823" height="40"><span>Xác minh mã OTP của bạn</span></h4>';
    ...
    // Create form group input & label
    $phoneNumber = $formGroup.clone().append($phoneNumber).append($label.clone().attr("for", "Phone").text("Số điện thoại của bạn"));
    $fnbCode = $formGroup.clone().append($fnbCode).append($label.clone().attr("for", "FnBCode").text("Mã xác minh")).append($errorList);
    $wrapperActions.append($actions);

    // Add input to form
    $form.append($phoneNumber).append($fnbCode).append($noti).append($wrapperActions);
...
function modalCreateStatus(status, url) {
    var $modal = $('<div id="modalCreateStatus" class="modal fade modal-register-success-default" role="dialog">'
        + '<div class="modal-dialog modal-dialog-centered" role="document"><div class="modal-content"><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span class="ti-close"></span></button></div></div>'),
        $modalContent = $modal.find('.modal-content');

    // Add form vao modal
    if (status == 'failed') {
        var $htmlBody = '<img src="/Themes/Portal/Default/StylesV2/images/fnb/icon-times-popup.png" alt="Thất bại">'
            + '<h5>Có lỗi xảy ra trong quá trình đăng ký. <span>Vui lòng thử lại sau!</span></h5>';
        $modalContent.addClass('modal-failed');
        $modalContent.append($htmlBody);
    } else {
        var $htmlBody = '<h5 style="margin-top: 30px;">Đăng ký dùng thử Sapo FnB thành công!</h5>'
            + '<p class="text-redirect">Chuyển hướng đến nhà hàng của bạn sau</p>'
...
function resendOTP(elem) {
    ...
    $.ajax({
        ...
        error: function (data) {
            $(elem).find('svg').hide();
            $form.find('.has-error').html('<span class="help-block">Có lỗi xảy ra khi gửi lại mã OTP. Vui lòng thử lại sau!</span>');
            return false;
        }
    });
}
...
function verifiedOTP(elem) {
    ...
    if (fnbCode == '' || fnbCode == null) {
        $(elem).find('svg').hide();
        $form.find('.has-error').html('<span class="help-block">Bạn chưa nhập mã xác minh</span>');
    ...
    if (fnbCode != null & fnbCode != '') {
        $.ajax({
            ...
            success: function (data) {
                if (data.error != null && data.error != '') {
                    $form.find('.has-error').html('<span class="help-block">' + data.error + '</span>');
            ...
            error: function (data) {
                $(elem).find('.btn-registration').find('svg').hide();
                $form.find('.has-error').html('<span class="help-block">Có lỗi xảy ra. Vui lòng thử lại sau!</span>');
                return false;
            }
        });
    } else {
